    %% Function to correct the OFDM symbol based on the centre Frequency as per sec 5.4 of 38.211
    % Phase Pre compensation is being done in the frequency domain
    ...
    %
 function [compensatedData,symbolPhases] = wnNrPhyPhasePreCompensation(rotationIn,  ... % Mapper output (1 slot)
                                                            bwFftCp, ...
    ...
    symbolPhases = 2* pi * (-t_mu_startSlot - t_u_cp(slotNumber*14 + 1 : (slotNumber+1)*14));

    % Phase Compensation
    for symIdx = 1:14
        compensatedData(:,symIdx) = rotationIn(:,symIdx).* ....
